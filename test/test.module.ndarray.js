...
tape( 'a module instance has a `ndarray` method which has an arity of 4', function test( t ) {
	var mem;
	var mod;
...
tape( 'if provided an `N` parameter less than or equal to `0`, a module instance has a `ndarray` method which returns `0.0`', function test( t ) {
	var mem;
	var mod;
	var xp;
	var y;
...
tape( 'if provided an `N` parameter equal to `1`, a module instance has a `ndarray` method which returns the absolute value of the first element', function test( t ) {
	var mem;
	var mod;
	var xp;
	var y;
...
tape( 'a module instance has a `ndarray` method which supports a `stride` parameter', function test( t ) {
	var mem;
	var mod;
	var xp;
	var y;
	...
	y = mod.ndarray( 4, xp, 2, 0 );
	t.strictEqual( y, 9.0, 'returns expected value' );

	t.end();
});

tape( 'a module instance has a `ndarray` method which supports a negative `stride` parameter', function test( t ) {
	var mem;
	var mod;
	var xp;
	var y;
